...
namespace Alchemist
{
    ...
    public partial class DataController : IDisposable
    {
        ...
        /// <summary>
        /// 通信クラスのMemWriteを使用し、指定したメモリアドレスの値を設定する
        /// </summary>
        /// <param name="Address"></param>
        /// <param name="MemVal"></param>
        /// <returns></returns>
        ...
        {
            return machineConnector.MemWrite(Address, MemVal);
        }

        /// <summary>
        /// 通信クラスのMemSendを使用し、指定したメモリアドレスの値を送る
        /// </summary>
        /// <param name="Address"></param>
        ...
        {
            return machineConnector.MemSend(Address, SendCount, ForceFlg);
        }
        ...
        public int BankDataLoad(int BankNo)
        {
            ...
            // 加工モードをバンクモードにする
            SetWorkMode(SystemConstants.WORKMODE_BANK);
            ...
            int resultWriteWD;
#else
            // CRIMP1_SW_BTN, CRIMP2_SW_BTNは強制的にOFFにする
            int resultWriteWD = WritePushBtn(SystemConstants.CRIMP1_SW_BTN, SystemConstants.BTN_OFF, false, false);
            if (resultWriteWD == SystemConstants.ERR_BANK_FILE_ERROR)
            {
                return resultWriteWD;
            }
            resultWriteWD = WritePushBtn(SystemConstants.CRIMP2_SW_BTN, SystemConstants.BTN_OFF, false, false);
            if (resultWriteWD == SystemConstants.ERR_BANK_FILE_ERROR)
            {
                return resultWriteWD;
            }
            ...
            {
                // Typeが加工値だった場合
                if (bankData[i].Type == SystemConstants.WORKMEM_TYPE_WORKDATA)
                {
                    ...
                    // 加工値を復元する
                    resultWriteWD = WriteWorkData(bankData[i].ID, Convert.ToDouble(bankData[i].value), false);
                ...
                // Typeが加工ボタンだった場合
                else if (bankData[i].Type == SystemConstants.WORKMEM_TYPE_WORKBTN)
                {
                    // WriteWorkDataを使用して、共有メモリに反映させる
                    int btnState = int.Parse(bankData[i].value);

                    // ボタンデータを復元する（関連動作は行わない）
                    resultWriteWD = WritePushBtn(bankData[i].ID, btnState, false, false);
                    if (resultWriteWD == SystemConstants.ERR_BANK_FILE_ERROR)
                    {
                        return resultWriteWD;
                    }
        ...
        /// <summary>
        /// WorkIDの加工値のフォーマットに合わせて、Valueを変換して、WorkDataStrに入れて返す
        /// </summary>
        /// <param name="WorkID"></param>
        /// <param name="Value"></param>
        /// <param name="WorkDataStr"></param>
        /// <returns></returns>
        public int ValueChangeToWorkIDFormatStr(int WorkIDType, int WorkID, double Value, ref string WorkDataStr)
        {
            MemAllocStruct memAllocStruct = null;
            WorkDataStr = "";
            ...
            {
                // メモリ割当データを使用し、WorkIDに対応する共有メモリのアドレスを取得
                memAllocStruct = memalloc.GetEntry(WorkIDType, WorkID);
            }
            // WorkIDがメモリ割当データに存在していない場合
            catch (MemoryAllocationDataException)
            {
            ...
            // メモリ割当てデータのVALUE_FACTORに対応する文字列に変換
            WorkDataStr = Utility.GetWorkDataString(Value, memAllocStruct.ValueFactor);
            // DCPF_SUCCESSを返す。
            return SystemConstants.DCPF_SUCCESS;
        }
...
#if FOR_JN03SDWP
                ErrBit[i] = Utility.ExchangeWordData(ErrBit[i]);
#elif FOR_JN03SDGP
                ErrBit[i] = Utility.ExchangeWordData(ErrBit[i]);
#elif FOR_JN03SDC
                ErrBit[i] = Utility.ExchangeWordData(ErrBit[i]);
        ...
        /// <summary>
        /// マシンステータスのメモリ範囲をロードする
        /// </summary>
        /// <param name="sendFlag"></param>
        /// <returns></returns>
        public int MachineStatusRead(bool sendFlag = true)
        {
            int sendCount = 0;
            int address;

            //マシン状態を取得する
            for (int i = 0; i < SystemConstants.MACHINE_STATUS_COUNT; i++)
            {
                address = SystemConstants.MACHINE_STATUS + i;
        ...
        /// </summary>
        /// <returns></returns>
        public int LearnDataSave()
        {
            string[] itemKeys;
            itemKeys = learnDataStorage.GetSearchKeys();

            // 選択されている学習データを取得する
            return LearnDataSave(itemKeys);
        ...
        /// <summary>
        /// LearnDataReadを使用して取得したデータを反映加工値に反映させます。
        /// その際、ボタンの設定をハードコーディングで登録を行います。
        /// </summary>
        /// <param name="ItemKeys"></param>
        /// <returns></returns>
        public int LearnDataLoad(string[] ItemKeys)
        {
            LearnDataStruct[] learnData = null;
            MemAllocStruct memAllocStruct = null;
            double min = 0.0, max = 0.0;
            int result = SystemConstants.DCPF_SUCCESS;

            // ItemKeysにnullがあった場合、空文字に変える
            for (int i = 0; i < ItemKeys.Count(); i++)
                if (ItemKeys[i] == null) ItemKeys[i] = "";

            // 学習データを読込む
            result = LearnDataRead(ItemKeys, ref learnData);
            if (result == SystemConstants.ERR_LSPF_FILE_ERROR) return result;

            // 加工モードを学習データモードに変更する
            SetWorkMode(SystemConstants.WORKMODE_LEARN);

            // 加工値メモリに保存
            workDataMemory.SetArray(learnData);

            // 検索キーを変更する
            learnDataStorage.storeSearchKey(ItemKeys);

            int resultWDWrite = 0;
#if !HCSM40
            // 圧着機スイッチ1, 2は強制的にOFFにする
            resultWDWrite = WritePushBtn(SystemConstants.CRIMP1_SW_BTN, SystemConstants.BTN_OFF, false, false);
            if (resultWDWrite == SystemConstants.ERR_LSPF_FILE_ERROR) return resultWDWrite;
            resultWDWrite = WritePushBtn(SystemConstants.CRIMP2_SW_BTN, SystemConstants.BTN_OFF, false, false);
            if (resultWDWrite == SystemConstants.ERR_LSPF_FILE_ERROR) return resultWDWrite;
#endif
            ...
            {
                if (learnData[i].Type == SystemConstants.WORKMEM_TYPE_WORKDATA)
                {
                    // GetWorkDataRangeを使用してメモリ割当データの上下限値と比較
                    int resultWDrange = GetWorkDataRange(learnData[i].ID, ref min, ref max);

                    // WorkIDがメモリ割当データに存在していない場合
                    if (resultWDrange == SystemConstants.ERR_NO_WORK_ID) continue;

                    // 範囲外の値があった場合
                    if (!Utility.CheckStringRange(learnData[i].value, min, max))
                    {
                        // 当該項目には標準値を設定
                        memAllocStruct = memalloc.GetEntry(SystemConstants.WORKID_TYPE_WORKDATA, learnData[i].ID);
                        learnData[i].value = memAllocStruct.DefaultValue.ToString();

                        // ERR_LSPF_PARTS_RANGEを戻り値に設定
                        result = SystemConstants.ERR_LEARN_PARTS_RANGE;
                    }

                    // 加工値を復元する
                    resultWDWrite = WriteWorkData(learnData[i].ID, Convert.ToDouble(learnData[i].value), false);

                    // ファイルに書込みが出来ない場合は、エラーを返す
                    if (resultWDWrite == SystemConstants.ERR_LSPF_FILE_ERROR)
                    {
                        return resultWDWrite;
                    }
                }
                else if (learnData[i].Type == SystemConstants.WORKMEM_TYPE_WORKBTN)
                {
                    // WritePushBtnを使用して、共有メモリに反映させる
                    int btnState = int.Parse(learnData[i].value);

                    // ボタンデータを復元する (関連動作は行わない)
                    resultWDWrite = WritePushBtn(learnData[i].ID, btnState, false, false);
                    if (resultWDWrite == SystemConstants.ERR_LSPF_FILE_ERROR)
                    {
                        return resultWDWrite;
                    }
        ...
        /// <summary>
        /// LearnDataStorageを使用して、学習データのキー項目の一覧を取得する
        /// </summary>
        /// <param name="ItemKeyCode"></param>
        /// <returns></returns>
        public string[] GetLeanedItemKey(int ItemKeyCode)
        {
            string[] learnedItemKey = learnDataStorage.GetLearnedItemKey(ItemKeyCode);
            return learnedItemKey;
        }

        /// <summary>
        /// 補正値、タイミングをファイルに出力します。
        /// </summary>
        /// <param name="GroupType"></param>
        /// <param name="filename"></param>
        /// <returns></returns>
        public int ExportCurrentData(int GroupType, string filename)
        {
            return exportcurrentData(GroupType, filename);
        }

        /// <summary>
        /// メモリ割当データを使用し、加工値ID (WorkID) に対応する係数をFalueFactorにいれて返します。
        /// 正常に処理が行われた場合、DCPF_SUCCESSを返す。
        /// WorkIDがメモリ割当データに存在していない場合、エラーとしてERR_NO_WORK_IDを返す。
        /// </summary>
        /// <param name="WorkID">加工値ID</param>
        /// <param name="ValueFactor">加工値IDに対応する係数</param>
        /// <returns></returns>
        public int GetWorkDataValueFactor(int WorkID, ref int ValueFactor)
        {
            MemAllocStruct memAllocStruct = null;
            ...
            // メモリ割当データを使用し、加工値ID (WorkID) に対応する係数をValueFactorに入れる
            ValueFactor = memAllocStruct.ValueFactor;
        ...
        /// バンクデータ再読込み処理
        /// </summary>
        /// <returns>成功:SystemConstants.DCPF_SUCCESS;true</returns>
        public int BankDataReLoad()
        {
            // バンクデータロード
            try
            {
        ...
        /// </summary>
        /// <returns></returns>
        public string[] GetWireTypeNames()
        {
            return learnItemDataStorage.GetNames(SystemConstants.LEARN_ITEM_WIRETYPE);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public string[] GetCoreSizeNames()
        {
            return learnItemDataStorage.GetNames(SystemConstants.LEARN_ITEM_CORESIZE);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public string[] GetColor1Names()
        {
            return learnItemDataStorage.GetNames(SystemConstants.LEARN_ITEM_COLOR1);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public string[] GetColor2Names()
        {
            return learnItemDataStorage.GetNames(SystemConstants.LEARN_ITEM_COLOR2);
        }
        ...
        /// <returns></returns>
        public int GetWireTypeCode(string Name, ref string Code)
        {
            return learnItemDataStorage.GetCode(Name, ref Code, SystemConstants.LEARN_ITEM_WIRETYPE);
        }
        ...
        /// <param name="Code"></param>
        /// <returns></returns>
        public int WriteWireType(string Name, string Code)
        {
            return learnItemDataStorage.WriteItem(Name, Code, SystemConstants.LEARN_ITEM_WIRETYPE);
        }
        ...
        /// <param name="Name"></param>
        /// <returns></returns>
        public int DeleteWireType(string Name)
        {
            return learnItemDataStorage.DeleteItem(Name, SystemConstants.LEARN_ITEM_WIRETYPE);
        }
        ...
        /// <param name="Code"></param>
        /// <returns></returns>
        public int GetCoreSizeCode(string Name, ref string Code)
        {
            return learnItemDataStorage.GetCode(Name, ref Code, SystemConstants.LEARN_ITEM_CORESIZE);
        }
        ...
        /// <param name="Code"></param>
        /// <returns></returns>
        public int WriteCoreSize(string Name, string Code)
        {
            return learnItemDataStorage.WriteItem(Name, Code, SystemConstants.LEARN_ITEM_CORESIZE);
        }
        ...
        /// <param name="Name"></param>
        /// <returns></returns>
        public int DeleteCoreSize(string Name)
        {
            return learnItemDataStorage.DeleteItem(Name, SystemConstants.LEARN_ITEM_CORESIZE);
        }
        ...
        /// <param name="Code"></param>
        /// <returns></returns>
        public int GetColo1Code(string Name, ref string Code)
        {
            return learnItemDataStorage.GetCode(Name, ref Code, SystemConstants.LEARN_ITEM_COLOR1);
        }
        ...
        /// <param name="Code"></param>
        /// <returns></returns>
        public int WriteColor1(string Name, string Code)
        {
            return learnItemDataStorage.WriteItem(Name, Code, SystemConstants.LEARN_ITEM_COLOR1);
        }
        ...
        /// <param name="Name"></param>
        /// <returns></returns>
        public int DeleteColor1(string Name)
        {
            return learnItemDataStorage.DeleteItem(Name, SystemConstants.LEARN_ITEM_COLOR1);
        }
        ...
        /// <param name="Code"></param>
        /// <returns></returns>
        public int GetColo2Code(string Name, ref string Code)
        {
            return learnItemDataStorage.GetCode(Name, ref Code, SystemConstants.LEARN_ITEM_COLOR2);
        }
        ...
        /// <param name="Code"></param>
        /// <returns></returns>
        public int WriteColor2(string Name, string Code)
        {
            return learnItemDataStorage.WriteItem(Name, Code, SystemConstants.LEARN_ITEM_COLOR2);
        }
        ...
        /// <param name="Name"></param>
        /// <returns></returns>
        public int DeleteColor2(string Name)
        {
