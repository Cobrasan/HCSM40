...
namespace Alchemist
{
    public partial class iocheckfrm : Form
    {
        ...
        public void Initialize()
        {
            ...
            /* コンパイルオプションで、IOWRITEENABLE をつけた時だけ書込み可能にする */
#if IOWRITEENABLE
            textWriteValue.Visible = true;
            lblWriteValue.Visible = true;
            btnWrite.Visible = true;
#else
            textWriteValue.Visible = false;
            lblWriteValue.Visible = false;
        ...
        {
#if IOWRITEENABLE
            int address = 0, memVal = 0;


            memVal = int.Parse(textWriteValue.Text);
            address = int.Parse(textAddress.Text, System.Globalization.NumberStyles.HexNumber);

            Program.DataController.MemWrite(address, memVal);
            Program.DataController.MemSend(address, 1, true);
#endif
        }
