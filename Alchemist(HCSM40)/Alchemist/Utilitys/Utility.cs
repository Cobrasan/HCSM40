    ...
    public class Utility
    {
        ...
        public static String GetReturnCodeString(int ret)
        {
            switch (ret)
            {
                ...
                case SystemConstants.ERR_BANK_PARTS_BREAK:
                ...
                case SystemConstants.ERR_LEARN_PARTS_BREAK:
                case SystemConstants.ERR_LEARN_PARTS_RANGE:
                    return GetMessageString(SystemConstants.SYSTEM_MSG, SystemConstants.SYSTEM_MSG033);
        ...
        {
            const int msgRsSystem = 0;
            const int msgRsScreen = 1;
            const int msgRsWork = 2;
            const int msgRsCorrent = 3;
            const int msgRsTimming = 4;
            const int msgRsStatus = 5;
            const int msgRsIO = 6;
            const int msgRsMainte = 7;
            const int msgResourceArrCount = 8;

            string key = "";
            string value = "";
            ResourceManager man = Alchemist.MsgResource.ORIGINAL.ScreenMsg.ResourceManager;
            ResourceManager[] manarr = new ResourceManager[msgResourceArrCount];

            // リソースファイルを設定する
            manarr[msgRsSystem] = Alchemist.MsgResource.ORIGINAL.SystemScreenMsg.ResourceManager;

#if HCSM40
            manarr[msgRsScreen] = Alchemist.MsgResource.ORIGINAL.ScreenMsg.ResourceManager;
            manarr[msgRsWork] = Alchemist.MsgResource.ORIGINAL.WorkDataMsg.ResourceManager;
            manarr[msgRsCorrent] = Alchemist.MsgResource.ORIGINAL.CorrectDataMsg.ResourceManager;
            manarr[msgRsTimming] = Alchemist.MsgResource.ORIGINAL.TimingDataMsg.ResourceManager;
            manarr[msgRsStatus] = Alchemist.MsgResource.ORIGINAL.StatusDspMsg.ResourceManager;
            manarr[msgRsIO] = Alchemist.MsgResource.ORIGINAL.IOAssignMsg.ResourceManager;
            manarr[msgRsMainte] = Alchemist.MsgResource.ORIGINAL.MainteLogMsg.ResourceManager;
#endif
            ...
            switch (MessageType)
            {
                case SystemConstants.SYSTEM_MSG:
                    ...
                    man = manarr[msgRsSystem];
                    break;
                case SystemConstants.ERROR_MSG:
                    key = "ERR_" + string.Format("{0:X4}", MessageID);
                    man = manarr[msgRsScreen];
                    break;
                case SystemConstants.WORK_MSG:
                    key = "WORK_" + string.Format("{0:X4}", MessageID);
                    man = manarr[msgRsWork];
                    break;
                case SystemConstants.CORRECT_MSG:
                    key = "CORR_" + string.Format("{0:X4}", MessageID);
                    man = manarr[msgRsCorrent];
                    break;
                case SystemConstants.TIMMING_MSG:
                    key = "TIMM_" + string.Format("{0:X4}", MessageID);
                    man = manarr[msgRsTimming];
                    break;
                case SystemConstants.STATUS_DISPLAY_MSG:
                    key = "STA_" + string.Format("{0:X4}", MessageID);
                    man = manarr[msgRsStatus];
                    break;
                case SystemConstants.MAINTELOG_MSG:
                    key = "MLOG_" + string.Format("{0:X4}", MessageID);
                    man = manarr[msgRsMainte];
                    break;
                case SystemConstants.IO_ASSIGN_MSG:
                    key = "IO_" + string.Format("{0:D3}", MessageID);
                    man = manarr[msgRsIO];
                    break;
            }
            ...
            if (null == value)
            {
                ...
                if (MessageType == SystemConstants.IO_ASSIGN_MSG)
                    value = "";
            }
        ...
        public static void ShowErrorCode(int ret)
        {
            ...
            // バンクデータ、学習データに欠損や範囲外の値があった場合は、情報としてメッセージを表示
            if (ret == SystemConstants.ERR_BANK_PARTS_BREAK ||
                ret == SystemConstants.ERR_BANK_PARTS_RANGE ||
                ret == SystemConstants.ERR_NO_BANK_DATA ||
                ret == SystemConstants.ERR_LEARN_PARTS_BREAK || ret == SystemConstants.ERR_LEARN_PARTS_RANGE)
            {
        ...
        // データの上位と下位データを入れ替える
        public static int ExchangeWordData(int Source)
        {
            int uByte, lByte;
            lByte = (Source & 0x00FF);
            uByte = (Source >> 8);
            ...
            // ビットを入れ替える
            int dstWord = (lByte << 8) + uByte;

            return dstWord;
        }

        /// <summary>
        /// グループタイプからメッセージタイプを変換する
        /// </summary>
        /// <param name="GroupType"></param>
        /// <returns></returns>
        public static int ConvertGroupTypeToMsgType(int GroupType)
        {
            switch (GroupType)
            {
                case SystemConstants.WORKID_TYPE_CORRECTDATA:
                    return SystemConstants.CORRECT_MSG;
                case SystemConstants.WORKID_TYPE_TIMINGDATA:
                    return SystemConstants.TIMMING_MSG;
                case SystemConstants.WORKID_TYPE_WORKDATA:
                    return SystemConstants.WORK_MSG;
                default:
